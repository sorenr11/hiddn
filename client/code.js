$(document).ready(function() {
    ...
    //spoiler, this does work
    //$('#status-update-form2 input[type="text"]').val(Math.random().toString(36).substring(7));

    // Setup DOM event listeners.
    $('#status-update-form input[type="text"]').on('keydown', function(e) {
        // The context of the message is determined by our state.
        var message = $(this).val();
        var key = $('#status-update-form2 input[type="text"]').val();
    ...
    $('#status-update-form button').on('click', function(e) {
        ...
        socket.emit('get_messages', {
            'key' : $('#status-update-form2 input[type="text"]').val()});

    });

    //set up the original key
    $('#status-update-form2 input[type="text"]').val(Math.random().toString(36).substring(7));

    $('#status-update-form2 button').on('click', function(e) {
        $('#status-update-form2 input[type="text"]').val(Math.random().toString(36).substring(7));
    });
